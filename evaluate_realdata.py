...
def eval_rl_on_real_data(model, price_paths, n_episodes=300):
    """
    Evaluate RL hedging on real price paths.
    The env samples a random path from price_paths on each reset.
    """
    errors = []
    costs = []
...
if __name__ == "__main__":
    ...
    model = PPO.load("ppo_hedging_realdata")   
    rl_errors, rl_costs = eval_rl_on_real_data(model, price_paths, n_episodes=300)
    print("RL (real-data): mean error =", rl_errors.mean(),
          "std =", rl_errors.std(),
          "mean cost =", rl_costs.mean())
    delta_errors, delta_costs = eval_delta_on_real_data(price_paths)
    print("Delta: mean error =", delta_errors.mean(),
          "std =", delta_errors.std(),
          "mean cost =", delta_costs.mean())
